...
test('Champion Point constellation locator focuses the selected route and supports map navigation', () => {
  const map = read('src/renderer/components/CPConstellationMap.jsx')
  const card = read('src/renderer/components/CPCard.jsx')
  const editor = read('src/renderer/pages/BuildChampionPointsPage.jsx')
  ...
  assert.match(map, /cpLayoutForTree/)
  assert.match(map, /ESO layout · Update 50/)
  assert.match(map, /works without addon sync/)
  assert.doesNotMatch(map, /getChampionGraph|shared profile sync|Approximate fallback|liveStars|constellationX|constellationY/)
  assert.match(editor, /canonical Update 50 ESO constellation geometry bundled with ATTB/)
  assert.match(editor, /Addon sync is not required for map placement/)
  assert.doesNotMatch(editor, /offline fallback|liveStars|getChampionGraph/)
