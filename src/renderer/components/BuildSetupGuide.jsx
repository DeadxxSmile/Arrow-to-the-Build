...
const DOCUMENTS = [
  ...
  ['research_audit', 'ESO Systems Audit', 'Why Schema 4 covers current ESO build and character systems.'],
  ['patch_maintenance', 'Patch Maintenance', 'Update catalogs and builds safely when ESO changes.']
]
...
export default function BuildSetupGuide({ flash }) {
  ...
  return <section className="panel guide-reader">
    <div className="guide-reader-head"><div><span className="eyebrow">Complete offline documentation</span><h2>Build Editor &amp; JSON guides</h2></div><label className="guide-search"><span>Search this page</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder="guided builds, JSON, skill IDs…" /></label></div>
