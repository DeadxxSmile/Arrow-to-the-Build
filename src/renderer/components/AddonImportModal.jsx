    ...
    {!setupOpen && <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose() }}>
      <section className="modal addon-import-modal" role="dialog" aria-modal="true" aria-labelledby="addon-import-title">
        ...
        {selected ? <>
          ...
          {selected.possible_match_id && <section className="addon-link-choice"><span className="eyebrow">Possible existing profile</span><h3>ATTB already has “{selected.possible_match_name}”</h3><p>Choose whether this ESO character should control that existing profile or be imported separately.</p>
            <label><input type="radio" name="addon-import-choice" checked={choice === 'link'} onChange={() => setChoice('link')} /><span><b>Link the existing ATTB character</b><small>Its selected build, notes, and build progress remain intact. Live ESO values begin syncing.</small></span></label>
