  ...
  return createPortal(<div className="preview-modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
    <section className="preview-modal" role="dialog" aria-modal="true" aria-labelledby="build-preview-title">
      ...
      <div className="preview-modal-scroll">
        ...
        {firstPhase && <section className="preview-section"><div className="section-head"><div><span className="eyebrow">First build phase</span><h2>{firstPhase.name || firstPhase.id}</h2><p>{firstPhase.overview || ''}</p></div></div><PreviewBar title="Front Bar" bar={firstPhase.front_bar} />{Number(metadata.bar_count || 2) > 1 && <PreviewBar title="Back Bar" bar={firstPhase.back_bar} />}</section>}
        {finalPhase && finalPhase !== firstPhase && <section className="preview-section"><div className="section-head"><div><span className="eyebrow">Final build phase</span><h2>{finalPhase.name || finalPhase.id}</h2><p>{finalPhase.overview || ''}</p></div></div><PreviewBar title="Front Bar" bar={finalPhase.front_bar} />{Number(metadata.bar_count || 2) > 1 && <PreviewBar title="Back Bar" bar={finalPhase.back_bar} />}</section>}
