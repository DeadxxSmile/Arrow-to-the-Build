...
export default function StatusPage() {
  ...
  return <div className="page current-levels-page">
    <div className="page-title"><span className="eyebrow">Current numeric progress</span><h1>Current levels</h1><p>{character.addon_sync?.linked ? 'Values below are synced from ESO. Override mode can temporarily replace individual fields without losing the synced value underneath.' : 'Record the numbers this character actually has right now. Build recommendations stay on Basic Setup, Skills & Passives, and Champion Points.'}</p></div>
    {character.addon_sync?.linked && <div className="sync-status-banner"><span className="sync-dot" /><div><b>ESO character linked</b><small>{character.addon_sync.world_name} · Last snapshot {character.addon_sync.captured_at ? new Date(character.addon_sync.captured_at * 1000).toLocaleString() : 'not yet written'}</small></div>{syncedLocked && <em>Overrides off</em>}</div>}
    ...
    <section className="panel numeric-tracking-panel">
      ...
      <div className="numeric-groups">{skillGroups.map(([group, lines]) => <section className="numeric-group" key={group}><header><h3>{group}</h3><span>{lines.length} line{lines.length === 1 ? '' : 's'}</span></header><div className="numeric-row-list">{lines.map(line => <label className="numeric-row" key={line.id}><span><b>{line.name}</b><small>{line.tracked_only ? 'Personal tracking' : 'Build-related line'}{character.addon_sync?.linked ? ` · ESO snapshot: ${character.addon_sync.live?.skill_ranks?.[line.id] ?? character.skill_ranks[line.id] ?? 0}` : ''}</small></span><div className="synced-control"><NumberStepper value={character.skill_ranks[line.id] ?? 0} min={0} max={line.max || 50} onChange={rank => setSkillRank(line.id, rank)} label={`${line.name} rank`} disabled={syncedLocked} /><OverrideResetButton fieldPath={`skill_ranks.${line.id}`} compact /></div></label>)}</div></section>)}</div>
    </section>
