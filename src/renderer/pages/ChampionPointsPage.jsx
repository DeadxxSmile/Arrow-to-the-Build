...
function LiveChampionState({ character }) {
  ...
  return <section className="panel live-observed-panel">
    ...
    <div className="live-observed-grid">{disciplines.map((discipline, index) => {
      ...
      <div className="live-observed-list">{(discipline.stars || []).some(star => Number(star.points || 0) > 0) ? (discipline.stars || []).filter(star => Number(star.points || 0) > 0).map(star => <div key={star.skillId}><b>{star.points || 0}/{star.maximumPoints || '?'}</b><span>{star.name}{star.slottable ? ' · slottable' : ''}</span></div>) : <div><b>Stars</b><span>No invested stars reported</span></div>}</div>
    </article>})}</div>
...
export default function ChampionPointsPage() {
  ...
  if (tree) return <div className="page champion-points-page v3-champion-points-page">
    ...
    <CPCard tree={tree} plan={plans[tree]} total={character[FIELD[tree]] || 0} onChange={setTree(tree)} detailed disabled={syncedLocked} observedChampion={character?.addon_sync?.observed?.champion || null} />
  </div>

  return <div className="page champion-points-page">
    <div className="page-title"><span className="eyebrow">Character-specific allocation</span><h1>Champion Points</h1><p>Enter or sync the total earned in each constellation. ATTB turns those budgets into an unlock-aware route, including prerequisite nodes, first-pass milestones, and exactly what to spend next.</p></div>
    ...
    <div className="cp-overview-grid">{TREES.map(key => <CPCard key={key} tree={key} plan={plans[key]} total={character[FIELD[key]] || 0} onChange={setTree(key)} disabled={syncedLocked} observedChampion={character?.addon_sync?.observed?.champion || null} />)}</div>
    <section className="panel cp-how-it-works"><div><span className="eyebrow">How allocation works</span><h2>Budget in, exact route out</h2></div><div className="cp-how-grid">
      <article><b>1</b><h3>Required connections</h3><p>ATTB inserts the real ESO prerequisite path and spends only enough to open the next connection.</p></article>
