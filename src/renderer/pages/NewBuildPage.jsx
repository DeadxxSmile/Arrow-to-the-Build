...
import {
  buildEditorGuidance, buildResourceOptions, buildRoleOptions, classes, races, recommendedRace, resourceGuidance, roleDefaults, roleGuidance
...
export default function NewBuildPage() {
  ...
  const [form, setForm] = useState(() => ({
    ...
    alliance: 'Ebonheart Pact', mundus: 'The Thief', bar_count: 2, starting_point: 'new_character', class_style: 'pure_class',
  ...
  return <div className="page new-build-page">
    ...
    <div className="new-build-choice-grid">
      <article className={`panel new-build-choice ${showGuided ? 'selected' : ''}`}><span className="build-kind editable">Recommended</span><div><h2>Guided Build</h2><p>Answer the important identity, role, resource, and starting-point questions. ATTB creates a valid class-specific scaffold for a new character, Level 50 transition, or CP160+ rebuild.</p></div><button type="button" className="btn primary" onClick={() => setShowGuided(value => !value)}>{showGuided ? 'Hide Guided Setup' : 'Start Guided Setup'}</button></article>
    ...
    {showGuided && <section className="panel guided-build-form section-block">
      ...
      <div className="guided-question"><FieldLabel guidance={showGuidance ? { title: 'Primary role', summary: roleGuidance(form.primary_role)?.summary } : null}>Primary role</FieldLabel><ChoiceChips name="Primary role" single values={[form.primary_role]} options={buildRoleOptions} onChange={changeRole} /></div>
      <div className="guided-question"><FieldLabel guidance={showGuidance ? { title: 'Primary resource', summary: resource.summary, common: resource.races?.map(race => `Common race: ${race}`) || [], notes: ['Role and encounter needs matter more than following a class stereotype.'] } : null}>Primary resource</FieldLabel><ChoiceChips name="Primary resource" single values={[form.resource]} options={buildResourceOptions} onChange={changeResource} /></div>
      ...
      <div className="guided-question"><FieldLabel guidance={showGuidance ? { title: 'Starting point', summary: 'Tell ATTB whether the build must teach 1-50 progression or is meant for an existing Level 50 / CP160+ character. This controls which leveling content is expected, not what the player is allowed to author.' } : null}>Who is this build for?</FieldLabel><ChoiceChips name="Build starting point" single values={[form.starting_point]} options={[{ value: 'new_character', label: 'New character' }, { value: 'level_50', label: 'Existing Level 50' }, { value: 'cp160_plus', label: 'Existing CP160+' }]} onChange={next => patch({ starting_point: next[0] })} /></div>
