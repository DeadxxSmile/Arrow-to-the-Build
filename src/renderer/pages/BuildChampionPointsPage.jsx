...
function NodeEditor({ tree, plan, node, index, count, onPatch, onMove, onDelete }) {
  ...
  return <article className="cp-node-editor-row catalog-backed">
    ...
    <div className="cp-node-fields">
      ...
      {canonical?.path_verified !== true && <div className="cp-editor-route-hint warning"><b>Offline path not fully verified.</b> A synced addon 1.1.3+ character supplies ESO's live graph for exact routing and map placement.</div>}
    </div>
...
export default function BuildChampionPointsPage() {
  ...
  return <div className="page build-editor-form-page build-cp-editor-page">
    ...
    {showMap && <section className="panel cp-editor-map"><div className="section-head"><div><span className="eyebrow">Catalog-backed locator</span><h2>{LABELS[tree]} constellation</h2><p>This editor preview uses the offline fallback layout. Character Tracker replaces it with ESO-relative outer-constellation positions after an addon 1.1.3+ sync. Multi-star clusters are intentionally shown as one portal node rather than an inner mini-map.</p></div></div>{!!expanded.unresolvedPaths?.length && <div className="notice-banner warn-banner">{expanded.unresolvedPaths.length} target{expanded.unresolvedPaths.length === 1 ? '' : 's'} need live ESO graph verification before ATTB can guarantee their offline prerequisite route.</div>}<CPConstellationMap tree={tree} route={expanded.route} /></section>}
