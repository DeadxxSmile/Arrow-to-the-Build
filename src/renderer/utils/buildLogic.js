import { buildIndex, catalogLineMap, normalizeSkillName } from './catalogLogic.js'
...
export function currentPhase(build, level) {
  const phases = build?.phases || []
  const value = Number(level) || 0
  return phases.find(p => value >= Number(p?.min_level ?? 0) && value <= Number(p?.max_level ?? Infinity)) || phases.at(-1) || null
}

// Schema 3 stores a stable id for every individual equipment piece.
...
// Schema 3 rows carry catalog_skill_id. The name fallback only supports in-memory test fixtures.
