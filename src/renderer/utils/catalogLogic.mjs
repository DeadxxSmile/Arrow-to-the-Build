...
export function catalogSkillIdForName(value = '') {
  const target = normalizeSkillName(value)
  if (!target || target === 'empty') return null
  const hits = uniqueSkills.filter(({ skill }) => normalizeSkillName(skill.name) === target)
  return hits.length ? hits[0].skill.id : null
}
