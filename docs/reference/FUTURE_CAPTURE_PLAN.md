...
Direct capture is not part of ATTB v0.3.1. The current SQLite and catalog model is deliberately structured so later capture tools can write through the same validation layer used by manual entry.
